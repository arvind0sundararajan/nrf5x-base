...
{
    uint32_t i = 0;
    uint8_t init_digest[SHA256_DIGEST_LENGTH];
    ...
    // Calculate digest for init packet
...
{
    uint8_t * received_hmac = m_extended_packet + SHA256_DIGEST_LENGTH;
    ...
    // calculate HMAC from image
    if (!HMAC_SHA256_compute(p_image, image_len, (uint8_t *) HMAC_KEY_LOCATION, 8, calculated_hmac)) {
        return NRF_ERROR_INVALID_DATA;
    }

    if (!memcmp(received_hmac, calculated_hmac, SHA256_DIGEST_LENGTH)) {
        return NRF_ERROR_INVALID_DATA;
    }
